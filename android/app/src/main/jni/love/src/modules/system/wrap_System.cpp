...
int w_googlePlayConnect(lua_State *L)
{
	instance()->googlePlayConnect();
	return 0;
}

int w_isGooglePlayConnected(lua_State *L)
{
	luax_pushboolean(L, instance()->isGooglePlayConnected());
	return 1;
}
...
int w_incrementAchievement(lua_State *L)
{
	std::string id = luax_checkstring(L, 1);
	int inc = (int) luaL_checknumber(L, 2);
	instance()->incrementAchievement(id, inc);
	return 0;
}
...
{
	std::string id = luax_checkstring(L, 1);
	double score = (double) luaL_checknumber(L, 2);
...
static const luaL_Reg functions[] =
{
	...
	{ "googlePlayConnect", w_googlePlayConnect },
    { "showLeaderboard", w_showLeaderboard },
    { "isGooglePlayConnected", w_isGooglePlayConnected },
    { "showAllLeaderboards", w_showAllLeaderboards },
    { "unlockAchievement", w_unlockAchievement },
    { "incrementAchievement", w_incrementAchievement },
